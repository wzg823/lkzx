      ...
      success:function(res){
        ...
        wx.request({
          ...
          success:function(res){
            ...
            wx.setStorage({
              key:"openId",
              data:res.data.resultMap.openId
            })
          }
        })
      }
  ...
  getCertifiMem() {
    ...
    confirmCertification() {
      ...
      AuthService.queryCertificationMember(this.data.phoneNmu).then((result) => {
        ...
        if (error.errMsg == 'request:fail') {
          ...
          console.log(this.data.memTelephone)
          this.setData({
            needVerCode:true,
            telephoneCode:this.data.memTelephone,
            codeText: '获取验证码'
          })
        }
        console.log(error);
    ...
    confirmPhoneCode() {
      homeService.uploadPhoneCode(this.data.cerInputNum, this.data.telephoneCode).then((result) => {
        ...
        // 获取 gym list 
        this.getGYMList();
        console.log(this.data.telephoneCode)
      }).catch((error) => {
        console.log(error);
      })
    },
      ...
      wx.getSetting({
        success: function(res){
          ...
          // 已经授权，可以直接调用 getUserInfo 获取头像昵称
            let infoDic = {
              memTelephone: _this.data.memTelephone,
              gym: _this.data.gym
            };
            wx.getUserInfo({
              success: function(res) {
                console.log(res.userInfo)
                homeService.uploadNewMem(infoDic,res.userInfo).then((result) => {
                  console.log(result)
                  _this.setData({
                    isCertificationMem: true,
                    modalHidden: true
